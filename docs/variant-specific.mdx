...
- A player is said to be _Loaded_ if they have a globally known play or a globally known safe discard.
- Players are allowed to give a _Save Clue_ to a card on chop, even if that clued player is _Loaded_.
- If a _Save Clue_ is given in this kind of situation, it is referred to as an _Early Save_. The clue is considered to be "early" because the _Loaded_ player will almost certainly play their playable card or discard their known trash before discarding their chop.
- In our group, _Early Saves_ are normal, and they don't cause anything special to happen. (Allowing _Early Saves_ is nice because it allows for _Discard Modulation._)

### The Loaded Play Clue (in Hard Variants) (LPC)

- First, see the section on the _[Early Save](#the-early-save)_.
- In [_Hard Variants_](#hard-variants--easy-variants), _Early Saves_ are not legal. Instead, if an _Early Save_ is given to a _Loaded_ player, then it is treated as a _Play Clue_ on that card.
- This is called a _Loaded Play Clue_ to distinguish it from a more ordinary _Play Clue_.
- For example, in a 3-player game of a _Hard_ variant:
  ...
  - However, Cathy also knows that _Early Saves_ are turned off in _Hard Variants_. Since Cathy is loaded with her playable 3, Bob's clue must have been a _Loaded Play Clue_.
...
- In such cases, we agree that this specific type of clue, if given to a player who is [_Loaded_](#the-early-save), must always be treated as a play clue. (This includes during the _Early Game_ and in _Double Discard Situations_.)
- For example, in a 3-player game with a gray suit:
  ...
  - Bob plays his _Finesse Position_ card. It is a gray 3 and it successfully plays.
...
- ALP does not apply to number 2, 5, or black color clues.
...
- If a player has two indistinguishable cards clued, it is hard to give a _Tempo Clue_ to the rightmost card. Thus, we agree that:
  - a color _Tempo Clue_ should focus the rightmost card.
  - a rank _Tempo Clue_ should focus the leftmost card.
